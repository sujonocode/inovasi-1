    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center flex-wrap">
                <h1 class="mb-3 mb-md-0">Data Kendala Kegiatan Statistik</h1>
                <div class="d-flex gap-2 flex-wrap">
                    <a href="<?= base_url('lantas/create') ?>"
                        class="btn btn-primary btn-sm flex-fill text-center"
                        style="min-width: 120px;"
                        title="Tambah SK Baru">
                        <i class="fa-solid fa-plus me-1"></i> Tambah
                    </a>
                    <a href="<?= base_url('lantas/export_xlsx') ?>"
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table id="example" class="table table-striped table-hover">
                    ...
                    <tbody>
                        <?php if (!empty($kendalas)): ?>
                            <?php foreach ($kendalas as $kendala): ?>
                                <tr>
                                    <td><?= $kendala['nomor'] ?></td>
                                    <td><?= $kendala['tanggal'] ?></td>
                                    <td><?= $kendala['perihal'] ?></td>
                                    <td><?= $kendala['catatan'] ?></td>
                                    <td><?= $kendala['created_by'] ?></td>
                                    <td>
                                        <a href="#" onclick="handleLinkClick('<?= $kendala['url'] ?>'); return false;"><i class="fa-solid fa-eye" title="Lihat"></i></a>
                                        <a href="/lantas/edit/<?= $kendala['id'] ?>"><i class="fa-solid fa-pen-to-square" title="Edit"></i></a>
                                        <a href="#" onclick="openDeleteModal(<?= $kendala['id'] ?>)"><i class="fa-solid fa-trash" title="Hapus"></i></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
...
<script>
    ...
    function openDeleteModal(kendalaId) {
        // Set the delete ID in a custom attribute, or store it globally
        const deleteUrl = "<?= base_url() ?>" + "lantas/delete/" + kendalaId;
