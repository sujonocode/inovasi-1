    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="form-section">
                ...
                <?php if (isset($jadwalPublikasi)): ?>
                    <form onsubmit="return validateCheckboxes()" action="<?= base_url('publikasi/update/' . $jadwalPublikasi['id']) ?>" method="POST">
                        <?= csrf_field() ?>
                        <div class="row form-group align-items-center flex-column flex-md-row">
                            <label for="nama" class="col-md-3 form-label">Nama:</label>
                            <div class="col-md-9">
                                <input id="nama" type="text" name="nama" class="form-control"
                                    value="<?= $jadwalPublikasi['nama'] ?>" required>
                            </div>
                        </div>
                        <div class="row form-group align-items-center flex-column flex-md-row">
                            <label for="tanggal" class="col-md-3 form-label">Tanggal Reminder:</label>
                            <div class="col-md-9">
                                <input id="tanggal" type="date" name="tanggal" class="form-control" value="<?= $jadwalPublikasi['tanggal'] ?>" required>
                            </div>
                        </div>
                        <div class="row form-group align-items-center flex-column flex-md-row">
                            <label for="waktu" class="col-md-3 form-label">Waktu Reminder:</label>
                            <div class="col-md-9">
                                <input id="waktu" type="time" name="waktu" class="form-control" step="1" value="<?= $jadwalPublikasi['waktu'] ?>" required>
                            </div>
                        </div>
                        <!-- <div class="row form-group align-items-center flex-column flex-md-row">
                        ...
                        <?php $pengingatArray = explode(',', $jadwalPublikasi['pengingat']); ?>
                        <div class="row form-group align-items-center flex-column flex-md-row">
                        ...
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-md-9">
                                <!-- <pre>< ?php print_r($jadwalPublikasi['kontak']); ?></pre> -->
                                <select name="kontak[]" id="kontak" class="form-select" multiple required>
                                    <?php foreach ($contacts as $contact): ?>
                                        <option value="<?= $contact['nomor'] ?>"
                                            <?php if (in_array($contact['nomor'], $jadwalPublikasi['kontak'])) echo 'selected'; ?>>
                        ...
                        <div class="row form-group align-items-center flex-column flex-md-row">
                            ...
                            <div class="col-md-9">
                                <textarea id="catatan" name="catatan" class="form-control" rows="3"
                                    required> <?= $jadwalPublikasi['catatan'] ?></textarea>
                            </div>
                        </div>
...
<script>
    ...
    // const kategori = "< ?= $jadwalPublikasi['kategori'] ?>";
    ...
    let pengingatString = '<?= json_encode($jadwalPublikasi['pengingat']) ?>';
...
<script>
    $(document).ready(function() {
        ...
        var selectedKontak = <?= json_encode($jadwalPublikasi['kontak']); ?>; // Get the selected contacts as a JavaScript array
